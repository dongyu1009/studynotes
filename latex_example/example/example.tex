...
\begin{document}
	...
	% subfig package is needed
	\begin{figure}[h]
		\centering 
		\subfigure[草地资源供给量空间分布]{ 
			\label{fig:capstd:a} %% label for first subfigure 
			\includegraphics[width=6in]{test} 
		} 
		\subfigure[单位面积草地可供给量标准差]{ 
			\label{fig:capstd:a} %% label for first subfigure 
			\includegraphics[width=0.5\textwidth]{test} 
		} 
		\subfigure[单位草地面积供给量变异系数]{ 
			\label{fig:capstd:b} %% label for second subfigure 
			\includegraphics[width=0.5\textwidth]{test} 
		} 
		\label{fig:capstd} 
		\caption{你好啊 啊}
